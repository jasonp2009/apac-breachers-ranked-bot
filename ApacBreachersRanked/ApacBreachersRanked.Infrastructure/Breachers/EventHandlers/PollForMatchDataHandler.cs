...
internal class PollForMatchDataHandler : INotificationHandler<PollForMatchDataEvent>
{
    ...
    private readonly ILogger<PollForMatchDataHandler> _logger;

    public PollForMatchDataHandler(
        BreachersDbContext dbContext,
        BreachersApiClient breachersApiClient,
        IMediator mediator,
        ILogger<PollForMatchDataHandler> logger)
    {
    ...
    public async Task Handle(PollForMatchDataEvent notification, CancellationToken cancellationToken)
    {
        try
        {
            ...
            breachersMatchDataEntity.QueueDomainEvent(new BreachersMatchDataReadyEvent { MatchId = breachersMatchDataEntity.MatchId });
            _dbContext.BreachersMatchData.Add(breachersMatchDataEntity);
            await _dbContext.SaveChangesAsync(cancellationToken);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
                "An error occurred when trying to poll for match data for {MatchId}",
