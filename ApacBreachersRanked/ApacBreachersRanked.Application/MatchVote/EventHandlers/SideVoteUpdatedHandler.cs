...
{
    public class SideVoteUpdatedHandler : INotificationHandler<SideVoteUpdatedEvent>
    {
        private readonly IDbContext _dbContext;
        private readonly IDiscordClient _discordClient;
        private readonly ILogger<MapVoteUpdatedHandler> _logger;
        public SideVoteUpdatedHandler(
        ...
        public async Task Handle(SideVoteUpdatedEvent notification, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    if (await channel.GetMessageAsync(matchVote.AwayVoteMessageId!.Value) is IUserMessage message)
                    {
